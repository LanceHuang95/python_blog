...
## 编写装饰函数 @get()
def get(path): # path: 装饰器get可以穿入的参数
    ## Define decorator @get('/path')
    def decorator(func):  # func:被装饰的函数
        @functools.wraps(func)
        def wrapper(*args, **kw): # args:被装饰的函数参数
            return func(*args, **kw)
...
# all keyword-only arguments without default values
# for example: def foo(a, b, *, c, d=10)    args = ('c')
    ...
    async def __call__(self, request):
        ...
        if self._has_var_kw_arg or self._has_named_kw_args or self._required_kw_args:
            ...
            if request.method == 'GET':
                ...
                if qs:
                    kw =dict()
                    # parse_qs:Parse a query string given as a string argument (data of type application/x-www-form-urlencoded). 
...
def add_route(app, fn):
    ...
    # ', '.join(inspect.signature(fn).parameters.keys() : 函数fn的形参列表
...
def add_routes(app, module_name):
    ...
    if n == -1:
        ...
        # 只获取handler.py中指定name的module
        name = module_name[n+1:]
        mod = getattr(__import__(module_name[:n], globals(), locals(), [name]), name)
    for attr in dir(mod):
        # _开头的方法或模块都为系统内置，跳过，只处理handler.py中实现的方法
        if attr.startswith('_'):
            continue
        fn = getattr(mod, attr)
        if callable(fn):
            # 含有这两个属性的方法 必定为 @get或者@post装饰过的方法
            method = getattr(fn, '__method__', None)
            path = getattr(fn, '__route__', None)
            if method and path:
                add_route(app, fn)
